...
public abstract class Graph {
    ...
    public abstract linealListClass<Edge> adjacentTo(int i);
    @Override
    public String toString(){
        String r = "";
        for (int i = 1; i<nVertices();i++){
            r += "\nVértice " + i + " ";
            linealListClass<Edge> pointer = adjacentTo(i);
            if (pointer.isEmpty()){
                r += "sin Adyacentes.\n";
            }else{
                r += "con Adyacentes: \n";
    ...
    protected void toArrayDFS(int origin, int res[]) {
        ...
        linealListClass<Edge> l = adjacentTo(origin);
    ...
    protected void toArrayBFS(int origin, int res[]) {
        ...
        while (!q.isEmpty()) {
            int u = q.deleteFirst();
            linealListClass <Edge> l = adjacentTo(u);
            for (l.setFirst(); !l.isLast(); l.keepMoving()) {
                Edge a = l.getMove();
                if (visited[a.dest] == 0) {
